...
PT_THREAD(initiate_registration(coap_session_t *session)) {
  ...
  /* sign our compressed ephemeral public key */
  {
    ...
    SHA_256.update(HASH_CTX_COMMA
                   ...
                   rap_context->msg.kno.cookie_size);
    if (!SHA_256.finalize(HASH_CTX_COMMA
                          hash)) {
      rap_context->result = 1;
      coap_log_err("initiate_registration: SHA_256.finalize failed\n");
      PT_EXIT(&rap_context->sub_pt);
    }
    PT_SPAWN(&rap_context->sub_pt,
             ecc_get_protothread(),
...
PT_THREAD(reconstruct_sms_public_key(coap_rap_context_t *rap_context)) {
  ...
  for (rap_context->reconstruct.i = 0;
    ...
    /* compute digest */
    {
      ...
      if (!SHA_256.hash(cert, cert_size, rap_context->reconstruct.cert_hash)) {
        rap_context->result = 1;
        coap_log_err("reconstruct_sms_public_key: SHA_256.hash failed\n");
        PT_EXIT(&rap_context->sub_pt);
      }
    }
...
PT_THREAD(verify_sm_report(coap_rap_context_t *rap_context)) {
  ...
  if (!SHA_256.finalize(HASH_CTX_COMMA
                        sm_report_hash)) {
    rap_context->result = 1;
    coap_log_err("verify_sm_report: SHA_256.finalize failed\n");
    PT_EXIT(&rap_context->sub_pt);
  }
  PT_SPAWN(&rap_context->sub_pt,
           ecc_get_protothread(),
...
PT_THREAD(verify_tee_report(
...
#endif /* ! WITH_TRAP || WITH_IRAP */
#if ! WITH_TRAP
  uint8_t sms_public_key[2 * ECC_CURVE_P_256_SIZE];
#endif /* ! WITH_TRAP */
  ...
  union {
    ...
    struct {
      uint8_t okm[2 * ECC_CURVE_P_256_SIZE];
#if ! WITH_IRAP
...

  {
    ...
    if (!SHA_256.finalize(HASH_CTX_COMMA
                          stack.s1.de)) {
      rap_context->result = 1;
      coap_log_err("verify_tee_report: SHA_256.finalize failed\n");
      PT_EXIT(&rap_context->sub_pt);
    }
  }
  PT_SPAWN(&rap_context->sub_pt,
  ...
  if (!sha_256_hkdf(
    ...
    rap_context->result = 1;
    coap_log_err("verify_tee_report: sha_256_hkdf failed\n");
    PT_EXIT(&rap_context->sub_pt);
  }
...
#if ! WITH_IRAP
  {
    ...
    sha_256_hmac_update(HASH_CTX_COMMA
                        ...
                        SHA_256_DIGEST_LENGTH);
    if (!sha_256_hmac_finish(HASH_CTX_COMMA
                             stack.s3.fhmqv_mic)) {
      coap_log_err("verify_tee_report: sha_256_hmac_finish failed\n");
      rap_context->result = 1;
      PT_EXIT(&rap_context->sub_pt);
    }
  }
  ...
  union {
    ...
    uint8_t tees_ephemeral_public_key[2 * ECC_CURVE_P_256_SIZE];
  ...
  /* derive secret */
  if (!sha_256_hkdf(rap_context->my.ephemeral_public_key_compressed,
                    sizeof(rap_context->my.ephemeral_public_key_compressed),
