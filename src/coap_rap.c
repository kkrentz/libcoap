...
coap_rap_initiate(coap_session_t *session,
  ...
  PROCESS_MUTEX_WAIT(&session->rap_pt, ECC.get_mutex());
...
on_cookie(coap_session_t *session,
  ...
  memcpy(rap_context->msg.kno.cookie, cookie, rap_context->msg.kno.cookie_size);
  rap_context->result = 1;
  rap_context->config->resume();
...
on_report(coap_session_t *session,
  ...
  /* parse */
  rap_context->sm.public_key_compressed[0] = 2 | (*payload & 1);
  rap_context->tee.ephemeral_public_key_compressed[0] = 2 | ((*payload & 2) >> 1);
  payload++;
