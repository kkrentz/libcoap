...
coap_rap_initiate(coap_session_t *session,
  ...
  while (1) {
    ecc_enable_result_t result = ECC.enable(&ecc_curve_p_256);
    if (!result) {
      break;
    }
    if (result == ECC_ENABLE_RESULT_ERROR) {
      coap_log_err("coap_rap_initiate: ECC.enable failed\n");
      goto error_1;
    }
    PT_YIELD(&session->attest_pt);
  }
